...
// 插入排序
void Gobang::sort(point * a, int n)
{
	int i, j;
	point key;
	for (i = 1; i < n; i++) {
...
///生成合法下法,返回合法的下棋点集合
int Gobang::generateBestMoves(int limit, point * bestMoves)
{
	int val, csize = 0;
	for (int i = 0; i<size; ++i)
		for (int j = 0; j < size; ++j)
...
int Gobang::getValue(int x, int y, int State)
{
...
void Gobang::getBestPos(int &x, int &y)
{
...
//判断游戏是否结束
void Gobang::isGameOver(int x, int y)
{
...
*
*/
void Gobang::removePiece(int x, int y)
{
	if (!board[x][y])
		return;
	else
...
///置换表相关实现函数
//写入置换表
void Gobang::writeToHashTable(int val, int depth)
{
...
//查询置换表
int  Gobang::searchHashTable(int depth)
{
	Hashe * e = &hashTable[zobristKey % hashSize];
